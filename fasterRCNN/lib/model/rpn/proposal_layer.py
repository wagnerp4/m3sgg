...
class _ProposalLayer(nn.Module):
    ...
    def forward(self, input):
        ...
        anchors = anchors.reshape(1, K * A, 4).expand(batch_size, K * A, 4)
        ...
        for i in range(batch_size):
            ...
            scores_single = scores_single[order_single].reshape(-1, 1)

            # 6. apply nms (e.g. threshold = 0.7)
            # 7. take after_nms_topN (e.g. 300)
            # 8. return the top proposals (-> RoIs top)
            keep_idx_i = nms(proposals_single, scores_single.squeeze(1), nms_thresh)
            keep_idx_i = keep_idx_i.long().reshape(-1)
    ...
    def _filter_boxes(self, boxes, min_size):
        ...
        keep = (ws >= min_size.reshape(-1, 1).expand_as(ws)) & (
            hs >= min_size.reshape(-1, 1).expand_as(hs)
        )
        return keep
