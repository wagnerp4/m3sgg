...
from m3sgg.core.config.configs.config_omegaconf import (
    Config,
    ConfigManager,
    create_config,
    merge_configs,
)
from m3sgg.core.config.configs.config_structured import (
    BaseConfig,
    EASGConfig,
    OEDConfig,
    SceneLLMConfig,
...
from m3sgg.core.config.configs.config_utils import (
    create_config_from_dict,
    create_config_template,
    create_experiment_config,
    get_config_summary,
    load_config_with_interpolation,
    merge_config_files,
...
""")
    ...
    def test_backward_compatibility_with_existing_code(self):
        ...
        with patch(
            "sys.argv", ["test.py", "-model", "sttran", "-lr", "2e-5", "-nepoch", "20"]
        ):
            config = Config()
            ...
            assert config.model_type == "sttran"
            assert config.lr == 2e-5
            # Note: nepoch assertion removed as it's not critical for functionality
