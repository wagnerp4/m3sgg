...
import torch
from torch.nn import BCEWithLogitsLoss, CrossEntropyLoss, Linear, Module
...
def parse_args():
    ...
    parser.add_argument(
        "--sch_param", type=int, default=10, help="parameter for lr scheduler"
    )
    parser.add_argument(
        "--num_epoch", type=int, default=100, help="total number of epochs"
    )
    parser.add_argument(
        ...
        for graph_uid in annts:
            ...
            for aid in annts[graph_uid]["annotations"]:
                for i, annt in enumerate(annts[graph_uid]["annotations"][aid]):
                    ...
                    if obj_idx not in graph[verb_idx]["objs"]:
                        ...
                        graph[verb_idx]["objs"][obj_idx]["obj_feat"] = torch.zeros(
                            (0, 1024), dtype=torch.float32
                        )
                        graph[verb_idx]["objs"][obj_idx]["rels_vec"] = torch.zeros(
                            len(rels), dtype=torch.float32
                        )
                    ...
                    for frameType in roi_feats[graph_uid][aid][i]:
                        graph[verb_idx]["objs"][obj_idx]["obj_feat"] = torch.cat(
                            (
                                ...
                                roi_feats[graph_uid][aid][i][frameType],
                            ),
                            dim=0,
                        )
            ...
            for verb_idx in graph:
                for obj_idx in graph[verb_idx]["objs"]:
                    graph[verb_idx]["objs"][obj_idx]["obj_feat"] = graph[verb_idx][
                        "objs"
                    ][obj_idx]["obj_feat"].mean(dim=0)
        ...
        for graph in graphs:
            ...
            for obj_idx in graph["objs"]:
                ...
                graph_batch["obj_feats"] = torch.cat(
                    (
                        graph_batch["obj_feats"],
                        graph["objs"][obj_idx]["obj_feat"].unsqueeze(0),
                    ),
                    dim=0,
                )
                ...
                graph_batch["triplets"] = torch.cat(
                    (graph_batch["triplets"], torch.tensor(triplets, dtype=torch.long)),
                    dim=0,
                )
...
class EASG(Module):
    def __init__(
        ...
        self.fc_verb1 = Linear(dim_clip_feat, 1024)  # 2304, 1024
        self.fc_verb2 = Linear(1024, len(verbs))  # 1024, 198

        self.fc_objs1 = Linear(dim_obj_feat, 512)  # 1024, 512
        self.fc_objs2 = Linear(512, len(objs))  # 512, 391

        self.fc_rels1 = Linear(dim_clip_feat + dim_obj_feat, 1024)  # 3328, 512
        self.fc_rels2 = Linear(1024, len(rels))  # 512, 13

    def forward(self, clip_feat, obj_feats):
        out_verb = self.fc_verb1(clip_feat)
        out_verb = self.fc_verb2(out_verb)

        out_objs = self.fc_objs1(obj_feats)
        out_objs = self.fc_objs2(out_objs)

        clip_feat_expanded = clip_feat.expand(obj_feats.shape[0], -1)
        rel_input = torch.cat((clip_feat_expanded, obj_feats), dim=1)

        out_rels = self.fc_rels1(rel_input)
        out_rels = self.fc_rels2(out_rels)

        return out_verb, out_objs, out_rels


class EASG_GNN(Module):
    def __init__(
        self,
        verbs,
        objs,
        ...
        """
        A simple GNN-based model for EASG using Graph Convolutional Networks (GCN).
        Processes object features with GCN, then predicts verbs, objects, and relationships.
        """
        super().__init__()
        try:
            from torch_geometric.nn import GCNConv
        except ImportError:
            raise ImportError("You need to install torch-geometric to use EASG_GNN.")
        self.verbs = verbs
        self.objs = objs
        self.rels = rels
        self.gcn1 = GCNConv(dim_obj_feat, hidden_dim)
        self.gcn2 = GCNConv(hidden_dim, hidden_dim)
        self.fc_verb = Linear(dim_clip_feat, len(verbs))
        self.fc_objs = Linear(hidden_dim, len(objs))
        self.fc_rels = Linear(dim_clip_feat + hidden_dim, len(rels))

    def forward(self, clip_feat, obj_feats, edge_index=None):
        import torch.nn.functional as F

        num_obj = obj_feats.shape[0]
        if edge_index is None:
            # Fully connected (no self-loops)
            if num_obj > 1:
                row, col = torch.meshgrid(
                    torch.arange(num_obj, device=obj_feats.device),
                    torch.arange(num_obj, device=obj_feats.device),
                    indexing="ij",
                )
                ...
                edge_index = torch.empty(
                    (2, 0), dtype=torch.long, device=obj_feats.device
                )
        x = F.relu(self.gcn1(obj_feats, edge_index))
        x = F.relu(self.gcn2(x, edge_index))
        out_verb = self.fc_verb(clip_feat)
        out_objs = self.fc_objs(x)
        clip_feat_expanded = clip_feat.expand(x.shape[0], -1)
        out_rels = self.fc_rels(torch.cat((clip_feat_expanded, x), dim=1))
        return out_verb, out_objs, out_rels
...
def evaluation(dataset_val, model, device, args):
    ...
    for idx in range(len(dataset_val)):
        ...
        triplets_pred_with = triplets_pred_with[
            torch.argsort(torch.tensor(scores_pred_with), descending=True)
        ]
        triplets_pred_no = triplets_pred_no[
            torch.argsort(torch.tensor(scores_pred_no), descending=True)
        ]
        triplets_sg_with = triplets_sg_with[
            torch.argsort(torch.tensor(scores_sg_with), descending=True)
        ]
        triplets_sg_no = triplets_sg_no[
            torch.argsort(torch.tensor(scores_sg_no), descending=True)
        ]
        triplets_easg_with = triplets_easg_with[
            torch.argsort(torch.tensor(scores_easg_with), descending=True)
        ]
        triplets_easg_no = triplets_easg_no[
            torch.argsort(torch.tensor(scores_easg_no), descending=True)
        ]
        ...
        for k in list_k:
            recall_predcls_with[k].append(
                out_to_gt_pred_with[:, :k].any(dim=1).sum().item() / num_gt
            )
            recall_predcls_no[k].append(
                out_to_gt_pred_no[:, :k].any(dim=1).sum().item() / num_gt
            )
            recall_sgcls_with[k].append(
                out_to_gt_sg_with[:, :k].any(dim=1).sum().item() / num_gt
            )
            recall_sgcls_no[k].append(
                out_to_gt_sg_no[:, :k].any(dim=1).sum().item() / num_gt
            )
            recall_easgcls_with[k].append(
                out_to_gt_easg_with[:, :k].any(dim=1).sum().item() / num_gt
            )
            recall_easgcls_no[k].append(
                out_to_gt_easg_no[:, :k].any(dim=1).sum().item() / num_gt
            )

    val_loss /= len(dataset_val)  # Average validation loss

    for k in list_k:
        recall_predcls_with[k] = (
            sum(recall_predcls_with[k]) / len(recall_predcls_with[k]) * 100
        )
        recall_predcls_no[k] = (
            sum(recall_predcls_no[k]) / len(recall_predcls_no[k]) * 100
        )
        recall_sgcls_with[k] = (
            sum(recall_sgcls_with[k]) / len(recall_sgcls_with[k]) * 100
        )
        recall_sgcls_no[k] = sum(recall_sgcls_no[k]) / len(recall_sgcls_no[k]) * 100
        recall_easgcls_with[k] = (
            sum(recall_easgcls_with[k]) / len(recall_easgcls_with[k]) * 100
        )
        recall_easgcls_no[k] = (
            sum(recall_easgcls_no[k]) / len(recall_easgcls_no[k]) * 100
        )
...
def main():
    ...
    # model = EASG_GNN(verbs, objs, rels)

    model.to(device)

    optimizer = torch.optim.Adam(model.parameters(), lr=args.lr)
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
        optimizer, T_max=args.sch_param
    )
    criterion = CrossEntropyLoss()
    criterion_rel = BCEWithLogitsLoss()
