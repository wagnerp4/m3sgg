...
def extract_computational_metrics_from_log(log_file):
    ...
    try:
        ...
        duration_match = re.search(r'Duration: ([\d.]+) seconds \(([\d.]+) minutes\)', content)
        if duration_match:
        ...
        if epoch_matches:
            ...
            if metrics['total_duration_seconds'] > 0:
                metrics['avg_epoch_time_seconds'] = metrics['total_duration_seconds'] / metrics['total_epochs']
                metrics['avg_epoch_time_minutes'] = metrics['avg_epoch_time_seconds'] / 60
        ...
        gpu_matches = re.findall(r'GPU info: .*?, ([\d.]+), ([\d.]+), ([\d.]+)', content)
        if gpu_matches:
        ...
        if metrics['total_duration_seconds'] > 0 and metrics['training_samples'] > 0:
            metrics['throughput_samples_per_second'] = metrics['training_samples'] / metrics['total_duration_seconds']
        ...
        for mode in modes:
            ...
            if os.path.exists(log_file):
                ...
                table_data.append({
                    'Model': model.upper(),
                    'Mode': mode,
                    'Duration (min)': 'N/A', 'Epochs': 0, 'Avg Epoch Time (min)': 'N/A',
                    'Memory Usage (MB)': 'N/A', 'Disk Usage (MB)': 'N/A',
                    'GPU Memory Used (MB)': 'N/A', 'GPU Memory Total (MB)': 'N/A',
                    'GPU Utilization (%)': 'N/A', 'Training Samples': 0, 'Test Samples': 0,
                    'Throughput (samples/s)': 'N/A', 'Final R@20': 'N/A', 'Final mR@20': 'N/A',
                    'Avg Train Loss': 'N/A', 'Final Val Loss': 'N/A'
                })
...
def create_efficiency_summary_table(df):
    ...
    for model in df['Model'].unique():
        ...
        avg_duration = pd.to_numeric(model_data['Duration (min)'], errors='coerce').mean()
        avg_epoch_time = pd.to_numeric(model_data['Avg Epoch Time (min)'], errors='coerce').mean()
        avg_memory = pd.to_numeric(model_data['Memory Usage (MB)'], errors='coerce').mean()
        avg_gpu_util = pd.to_numeric(model_data['GPU Utilization (%)'], errors='coerce').mean()
        avg_throughput = pd.to_numeric(model_data['Throughput (samples/s)'], errors='coerce').mean()
        ...
        summary_data.append({
            'Model': model,
            'Avg Duration (min)': f"{avg_duration:.2f}" if not pd.isna(avg_duration) else 'N/A',
            'Avg Epoch Time (min)': f"{avg_epoch_time:.2f}" if not pd.isna(avg_epoch_time) else 'N/A',
            'Avg Memory (MB)': f"{avg_memory:.1f}" if not pd.isna(avg_memory) else 'N/A',
            'Avg GPU Util (%)': f"{avg_gpu_util:.1f}" if not pd.isna(avg_gpu_util) else 'N/A',
            'Avg Throughput (samples/s)': f"{avg_throughput:.2f}" if not pd.isna(avg_throughput) else 'N/A',
...
def main():
    ...
    if not valid_durations.empty:
        ...
        print(f"Fastest training: {df.iloc[fastest_idx]['Model']} {df.iloc[fastest_idx]['Mode']} ({df.iloc[fastest_idx]['Duration (min)']} min)")
        print(f"Slowest training: {df.iloc[slowest_idx]['Model']} {df.iloc[slowest_idx]['Mode']} ({df.iloc[slowest_idx]['Duration (min)']} min)")
    ...
    if not valid_throughputs.empty:
        ...
        print(f"Most efficient: {df.iloc[most_efficient_idx]['Model']} {df.iloc[most_efficient_idx]['Mode']} ({df.iloc[most_efficient_idx]['Throughput (samples/s)']} samples/s)")
        print(f"Least efficient: {df.iloc[least_efficient_idx]['Model']} {df.iloc[least_efficient_idx]['Mode']} ({df.iloc[least_efficient_idx]['Throughput (samples/s)']} samples/s)")
