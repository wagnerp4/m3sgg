...
def extract_training_samples_from_log(log_file):
    ...
    try:
        ...
        train_samples_match = re.search(r'Using (\d+)/\d+\s+training samples', content, re.DOTALL)
        ...
        if metrics['training_samples'] > 0 and metrics['total_epochs'] > 0:
            metrics['total_samples_processed'] = metrics['training_samples'] * metrics['total_epochs']
        else:
        ...
        for mode in modes:
            ...
            if os.path.exists(log_file):
                ...
                table_data.append({
                    'Model': model.upper(),
                    'Mode': mode,
                    'Training Samples': 0, 'Test Samples': 0, 'Train Batches': 0, 'Test Batches': 0,
                    'Fraction Used': 0, 'Total Epochs': 0, 'Total Samples Processed': 0
                })
...
def create_summary_table(df):
    ...
    summary_data.append({
        'Metric': 'Total Test Samples (All Models)',
        'Value': total_test_samples
    })
    summary_data.append({
        'Metric': 'Total Train Batches (All Models)',
        'Value': total_train_batches
    })
    summary_data.append({
        'Metric': 'Total Test Batches (All Models)',
        'Value': total_test_batches
    })
    summary_data.append({
    ...
    for model in df['Model'].unique():
        ...
        summary_data.append({
            'Metric': f'{model} - Training Samples',
            'Value': model_training_samples
        })
        summary_data.append({
            'Metric': f'{model} - Test Samples',
            'Value': model_test_samples
        })
        summary_data.append({
...
def main():
    ...
    if len(unique_training_samples) == 1 and unique_training_samples[0] > 0:
        print(f"✓ All models used the same dataset size: {unique_training_samples[0]} training samples")
    else:
        print("⚠ Different models used different dataset sizes")
    
    if len(unique_fractions) == 1 and unique_fractions[0] > 0:
        print(f"✓ All models used the same fraction: {unique_fractions[0]} (1=all, 2=half, 5=20%, 10=10%)")
    else:
