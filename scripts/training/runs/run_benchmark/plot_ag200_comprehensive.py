...
def extract_metrics_from_log(log_file):
    ...
    try:
        ...
        for line in lines:
            # Extract train loss
            train_loss_match = re.search(r'Epoch (\d+) \| avg_train_loss=([\d.]+)', line)
            if train_loss_match:
                current_epoch = int(train_loss_match.group(1))
                if current_epoch not in epoch_metrics:
                    epoch_metrics[current_epoch] = {}
                epoch_metrics[current_epoch]['train_loss'] = float(train_loss_match.group(2))
            ...
            if val_loss_match:
                ...
                if current_epoch not in epoch_metrics:
                    epoch_metrics[current_epoch] = {}
                epoch_metrics[current_epoch]['val_loss'] = float(val_loss_match.group(2))
            
            # Extract R@K and mR@K metrics
            rk_match = re.search(r'(R@10|R@20|R@50|R@100|MR@10|MR@20|MR@50|MR@100): ([\d.]+)', line)
...
def create_plot1_all_models_r20(results, output_dir):
        ...
        for j, mode in enumerate(modes):
            if model in results and mode in results[model]:
                ...
                if data['epochs'] and data['R@20']:
                    plt.plot(data['epochs'], data['R@20'], 
    ...
    plt.title('R@20 Performance Across All Models and Modes', fontsize=14, fontweight='bold')
...
def create_plot2_tempura_all_rk(results, output_dir):
    ...
    plt.title('Tempura: All R@K Metrics Across All Modes', fontsize=14, fontweight='bold')
    plt.grid(True, alpha=0.3)
    plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
    plt.tight_layout()
...
def create_plot3_train_vs_val_loss(results, output_dir):
    ...
    fig.suptitle('Train Loss vs Validation Loss Across All Models and Modes', fontsize=16, fontweight='bold')
            ...
            if model in results and mode in results[model]:
                ...
                if data['epochs'] and data['train_loss'] and data['val_loss']:
                    ax.plot(data['epochs'], data['train_loss'], 
                           color=colors[i], marker='o', label='Train Loss', 
                           linewidth=2, markersize=4)
                    ax.plot(data['epochs'], data['val_loss'], 
                           color=colors[i], marker='s', label='Val Loss', 
...
def create_plot4_all_models_mr20(results, output_dir):
        ...
        for j, mode in enumerate(modes):
            if model in results and mode in results[model]:
                ...
                if data['epochs'] and data['MR@20']:
                    plt.plot(data['epochs'], data['MR@20'], 
                            color=colors[i], marker=markers[j], 
                            label=f'{model.upper()} {mode}', 
                            linewidth=2, markersize=6)
    ...
    plt.title('mR@20 Performance Across All Models and Modes', fontsize=14, fontweight='bold')
    plt.grid(True, alpha=0.3)
    plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
    plt.tight_layout()
            ...
            if model in results and mode in results[model]:
                ...
                if data['epochs'] and data['R@20']:
                    summary_data.append({
                        ...
                        'Final Train Loss': data['train_loss'][-1] if data['train_loss'] else 0,
                        'Final Val Loss': data['val_loss'][-1] if data['val_loss'] else 0
                    })
...
def main():
    parser = argparse.ArgumentParser(description='Generate comprehensive plots for AG200 benchmark')
    parser.add_argument('--results_dir', default='data/benchmark/ag200', 
                       help='Directory containing benchmark results')
    parser.add_argument('--output_dir', default='data/benchmark/ag200/plots', 
