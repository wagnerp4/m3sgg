...
from lib.datasets.easg import EASG, cuda_collate_fn
...
def main():
    ...
    den = 1  # fraction of the dataset to use for training
    train_size = len(EASG_dataset_train)
    test_size = len(EASG_dataset_test)
    train_subset_size = train_size // den
    test_subset_size = test_size // den
    ...
    dataloader_train = torch.utils.data.DataLoader(
        # shuffle=True, TODO: check randomness of the sampler
        EASG_dataset_train,
    ...
    logger.info(
        f"Using {train_subset_size}/{train_size} training samples and {test_subset_size}/{test_size} test samples"
    )
    ...
    object_detector = detector(
        train=True,
        object_classes=EASG_dataset_train.obj_classes,
        use_SUPPLY=True,
        mode=conf.mode,
    ...
    for epoch in range(1, conf.nepoch + 1):
        ...
        train_pbar = tqdm(
            range(1, len(dataloader_train) + 1),
            desc=f"Epoch {epoch}/{conf.nepoch} [Train]",
        )
        for b in train_pbar:
            ...
            entry["features_verb"] = copy.deepcopy(
                EASG_dataset_train.verb_feats[data[2]].cuda(0)
            )
            pred = model(entry)
            ...
            edge_label = -torch.ones(
                [len(pred["edge"]), len(EASG_dataset_train.edge_classes)],
                dtype=torch.long,
            ...
            if b % freq == 0:
                ...
                msg = "edge_loss: {:.3f}".format(losses_mean["edge_loss"])
                if conf.mode != "edgecls":
                ...
                logger.info(
                    f"epoch [{epoch:2d}/{conf.nepoch:2d}] | batch [{b:3d}/{len(dataloader_train):3d}] | {msg}"
                )

        torch.save(
            {"state_dict": model.state_dict()},
            os.path.join(conf.save_path, "model_{}.tar".format(epoch)),
        )
        ...
        with torch.no_grad():
            ...
            test_pbar = tqdm(
                range(1, len(dataloader_test) + 1),
                desc=f"Epoch {epoch}/{conf.nepoch} [Val]",
            )
            for b in test_pbar:
                data = next(test_iter)
                ...
                entry = object_detector(im_data, im_info, gt_grounding, im_all=None)
                entry["features_verb"] = copy.deepcopy(
                    EASG_dataset_test.verb_feats[data[2]].cuda(0)
                )
                pred = model(entry)
                ...
                for gcid in gcid_to_f_idx:
                    ...
                    scores_verb = pred["distribution_verb"][gcid_to_b_idx[gcid]].mean(
                        dim=0
                    )
