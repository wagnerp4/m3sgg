...
def simple_process_video_with_bboxes(
    input_video_path, output_video_path, max_frames=30
):
...
def simple_process_video_with_model(
    input_video_path, output_video_path, model_path, max_frames=30
):
    ...
    try:
        ...
        while frame_count < max_frames and cap.isOpened():
            ...
            try:
                ...
                if frame_count % 5 == 0:
                    print(
                        f"Frame {frame_count}: {frame_results.get('objects', 0)} objects detected"
                    )
        ...
        print(
            f"✅ Video processing completed: {processed_frames} frames processed, {detection_count} total detections"
        )
