            ...
            for annt in annts[split][graph_uid]["annotations"][aid]:
                ...
                for frameType in annt["bbox"]:
                    fname = args.fname_format.format(
                        clip_uid=clip_uid,
                        clip_frame_number=annts[split][graph_uid]["clip_frame_number"][
                            frameType
                        ],
                    )
                    ...
                    with torch.no_grad():
                        ...
                        H, W = img.shape[-2:]
                        boxes = torch.tensor(annt["bbox"][frameType]).to(
                            cfg.MODEL.DEVICE
                        )
                        boxes[:, ::2] *= W
                        boxes[:, 1::2] *= H
                        ...
                        features = [
                            features[f] for f in cfg.MODEL.ROI_HEADS.IN_FEATURES
                        ]
                        outputs = model.roi_heads.box_pooler(
