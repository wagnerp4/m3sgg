...
from lib.configs.config_omegaconf import (  # New config system
    ConfigManager,
    create_config,
)
from lib.configs.config_utils import get_config_summary, validate_config
...
def example_2_yaml_configuration():
    ...
    config_path = "lib/configs/sttran.yaml"
...
def example_3_structured_configuration():
    ...
    config = create_config(
        model_type="scenellm", lr=2e-5, nepoch=20, scenellm_training_stage="vqvae"
    )
    ...
    try:
        ...
        print(
            f"Learning rate after setting string: {config.lr} (type: {type(config.lr)})"
        )
...
def example_4_command_line_overrides():
    ...
    config = ConfigManager(config_path="lib/configs/base.yaml", cli_args=cli_args)

    print(f"Base config loaded from: lib/configs/base.yaml")
...
def example_5_experiment_management():
    ...
    from lib.configs.config_utils import (
    ...
    overrides = {"lr": 2e-5, "nepoch": 25, "save_path": "output/experiment_demo"}
