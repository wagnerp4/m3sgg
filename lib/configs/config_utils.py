...
def register_custom_resolvers():
    ...
    def resolve_model_param(
        model_type: str, param_name: str, default: Any = None
    ) -> Any:
    ...
    def resolve_dataset_path(
        dataset: str, datasize: str, base_path: str = "data"
    ) -> str:
            ...
            if not OmegaConf.is_missing(config, field_name):
                try:
                    ...
                    if not isinstance(value, field_type):
                        errors.append(
                            f"Field '{field_name}' should be of type {field_type.__name__}, got {type(value).__name__}"
                        )
                except ConfigKeyError:
                    errors.append(
                        f"Required field '{field_name}' not found for model type '{model_type}'"
                    )
...
def save_experiment_config(config: DictConfig, output_path: str):
    ...
    with open(script_path, "w") as f:
        ...
        f.write("from lib.configs.config_omegaconf import create_config\n\n")
...
def load_config_with_interpolation(
    config_path: str, interpolation_vars: Dict[str, Any] = None
) -> DictConfig:
