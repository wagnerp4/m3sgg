...
try:
    ...
    logging.warning(f"rouge_score library not available: {e}. Install with: pip install rouge-score")
...
try:
    ...
    logging.warning(f"sentence-transformers library not available: {e}. Install with: pip install sentence-transformers")
...
class SummarizationMetrics:
    ...
    def compute_rouge(self, predictions: List[str], references: List[str]) -> Dict[str, float]:
        ...
        avg_scores = {}
        for rouge_type in self.rouge_types:
            avg_scores[rouge_type] = sum(rouge_scores[rouge_type]) / len(rouge_scores[rouge_type])
        ...
        for pred, ref in zip(predictions, references):
            ...
            for n in range(1, 5):
                bleu_score = sentence_bleu([ref_tokens], pred_tokens, 
                                         weights=[1/n] * n, smoothing_function=smoothing)
                bleu_scores[f"bleu{n}"].append(bleu_score)
        
        # Compute averages
        avg_scores = {}
        for bleu_type in bleu_scores:
            avg_scores[bleu_type] = sum(bleu_scores[bleu_type]) / len(bleu_scores[bleu_type])
    ...
    def compute_semantic_similarity(self, predictions: List[str], references: List[str]) -> float:
        ...
        if not SENTENCE_TRANSFORMERS_AVAILABLE or self.sentence_model is None:
            logger.warning("Sentence transformers not available, returning 0.0 for semantic similarity")
            return 0.0
    ...
    def format_results(self, metrics: Dict[str, float], precision: int = 4) -> str:
        ...
        if other_metrics:
            ...
            for metric, score in other_metrics.items():
                lines.append(f"  {metric.replace('_', ' ').title()}: {score:.{precision}f}")
