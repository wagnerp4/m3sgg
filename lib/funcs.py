...
def assign_relations(prediction, gt_annotations, assign_IOU_threshold):
    ...
    for i, j in enumerate(gt_annotations):
        ...
        #
        assigned_labels[
            (FINAL_BBOXES[:, 0].cpu().numpy() == i).nonzero()[0][
                np.max(IOUs, axis=1) > 0.5
            ]
        ...
        for m, n in enumerate(gt_annotations[i]):
            if m == 0:
                ...
                if sum(IOUs[:, m] > assign_IOU_threshold) > 0:
                    candidate = IOUs[
                        :, m
                    ].argmax()  # [labels[np.where(IOUs_bool[:, m])[0]] == 1]
                    detector_found_idx.append(candidate)
                    gt_relations.append(n)
                    candidates.append(candidate)
                else:
                    supply_relations.append(
                        n
                    )  # no person box is found...i think it is rarely(impossible)
            else:
                if sum(IOUs[:, m] > assign_IOU_threshold) > 0:
                    ...
                    candidates.append(candidate)
                    assigned_labels[
                        (FINAL_BBOXES[:, 0].cpu().numpy() == i).nonzero()[0][candidate]
                    ] = n["class"]
                else:
                    # no overlapped box
