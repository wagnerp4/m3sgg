...
def optimistic_restore(network, state_dict):
    ...
    for name, param in state_dict.items():
        if name not in own_state:
            print(
                "Unexpected key {} in state_dict with size {}".format(
                    name, param.size()
                )
            )
            mismatch = True
            ...
            print(
                "Network has {} with size {}, ckpt has {}".format(
...
def load_net(fname, net):
    ...
    for k, v in list(net.state_dict().items()):
        ...
        if v.size() != param.size():
            print(
                "On k={} desired size is {} but supplied {}".format(
                    k, v.size(), param.size()
                )
            )
        else:
...
def clip_grad_norm(named_parameters, max_norm, clip=False, verbose=False):
    ...
    if verbose:
        print(
            "---Total norm {:.3f} clip coef {:.3f}-----------------".format(
                total_norm, clip_coef
            )
        )
