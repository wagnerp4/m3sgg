...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, src, prior, input_key_padding_mask, position_embed):
        query, key, value = self.W_q(src + prior), self.W_k(src + prior), self.W_v(src)
        global_src, global_attention_weights = self.self_attn(
            query=query + position_embed,
            key=key + position_embed,
            value=value,
    ...
    def forward(self, input, prior, input_key_padding_mask):
        ...
        for i, layer in enumerate(self.layers):
            output, local_attention_weights = layer(
                output, prior, input_key_padding_mask
            )
    ...
    def forward(self, input, prior, input_key_padding_mask, position_embed):
        ...
        for i, layer in enumerate(self.layers):
            output, global_attention_weights = layer(
                output, prior, input_key_padding_mask, position_embed
            )
...
class spatial_encoder(nn.Module):
    def __init__(
        ...
        self.initialPrior = torch.zeros(
            (obj_class_num, attention_class_num + spatial_class_num + contact_class_num)
        )
        ...
        self.state_fc = nn.Sequential(
            nn.Linear(attention_class_num + spatial_class_num + contact_class_num, 128),
            nn.Linear(128, 256),
            nn.Linear(256, 512),
            nn.Linear(512, 1936),
        )
    ...
    def forward(self, features, im_idx, entry, mode):
        ...
        )  # torch.mode->(tensor, longTensor), 返回众数以及众数下标
        frame_num = int(im_idx[-1] + 1)

        rel_input = torch.zeros([bbox_num, frame_num, features.shape[1]]).to(
            features.device
        )  # (bbox_num, frame_num, feature_dim)
        rel_prior = torch.zeros([bbox_num, frame_num, features.shape[1]]).to(
            features.device
        )  # (bbox_num, frame_num, feature_dim)

        priors = torch.zeros_like(features).to(
            features.device
        )  # (bbox_num, feature_dim)
        masks = torch.zeros([frame_num, bbox_num], dtype=torch.bool).to(
            features.device
        )  # (frame_num, bbox_num)
        ...
        for rel_idx in range(features.size(0)):
            ...
            label = (
                int(entry["labels"][bbox_idx].item())
                if mode == "predcls"
                else int(entry["pred_labels"][bbox_idx].item())
            )
            priors[rel_idx] = self.state_fc(self.initialPrior[label])

        for frame_idx in range(frame_num):
            rel_input[: torch.sum(im_idx == frame_idx), frame_idx, :] = features[
                im_idx == frame_idx
            ]
            masks[frame_idx, torch.sum(im_idx == frame_idx) :] = True

            if self.use_spatial_prior:
                rel_prior[: torch.sum(im_idx == frame_idx), frame_idx, :] = priors[
                    im_idx == frame_idx
                ]
        ...
        spatial_features = (
            (rel_output.permute(1, 0, 2))
            .contiguous()
            .view(-1, features.shape[1])[masks.view(-1) == False]
        )  # (rel_num, feature_dim)
        spatial_priors = (
            (rel_prior.permute(1, 0, 2))
            .contiguous()
            .view(-1, features.shape[1])[masks.view(-1) == False]
        )  # (rel_num, feature_dim)
...
class temporal_decoder(nn.Module):
    def __init__(
        ...
        self.specificPrior = torch.zeros(
            (
                obj_class_num,
                contact_class_num,
        ...
        for idx, prior in trainPrior.items():
            ...
            for c_rel_idx, c_rel_prior in zip(
                prior["c_rel_idx"], prior["specific_prior"]
            ):
                self.specificPrior[obj_cls_idx][c_rel_idx] = torch.tensor(c_rel_prior)

        self.initialPrior, self.specificPrior = (
            nn.Parameter(self.initialPrior),
            nn.Parameter(self.specificPrior),
        )
    ...
    def forward(self, features, contact_distribution, im_idx, entry, mode):
        ...
        temporal_priors, initialPrior, specificPrior = (
            torch.zeros_like(features).to(features.device),
            torch.zeros_like(features).to(features.device),
            torch.zeros_like(features).to(features.device),
        )
        ...
        for rel_idx in range(features.size(0)):
            ...
            obj_score = (
                entry["pred_scores"][bbox_idx]
                if (mode != "predcls") and (not self.training)
                else 1
            )
            ...
            if self.training:
                ...
                if self.pred_contact_threshold == 1.0:
                    history_anno_dict[label][frame_idx] = set(
                        entry["contact_gt"][rel_idx]
                    )
                else:
                    history_anno_dict[label][frame_idx] = (
                        {8}
                        if np.argmax(_contact_distribution[rel_idx]) == 8
                        else set(
                            np.where(
                                _contact_distribution[rel_idx]
                                >= self.pred_contact_threshold
                            )[0].tolist()
                        )
                        - {8}
                    )
            ...
            specificPrior[rel_idx] = (
                self.state_fc(torch.stack(embedding).mean(dim=0))
                if len(embedding) != 0
                else self.state_fc(self.initialPrior[label])
            )

            previous_frame_idx = np.where(
                history_score_dict[label][:frame_idx].cpu().numpy() != 0
            )[0].tolist()
            previous_frame_idx = (
                previous_frame_idx[-1] if len(previous_frame_idx) > 0 else frame_idx
            )
            previous_frame_idx = (
                previous_frame_idx if frame_idx - previous_frame_idx <= 2 else frame_idx
            )

            embedding = []
            for c_rel_idx in history_anno_dict[label][previous_frame_idx]:
                embedding.append(self.specificPrior[label][c_rel_idx])
            temporal_priors[rel_idx] = (
                self.state_fc(torch.stack(embedding).mean(dim=0))
                if len(embedding) != 0
                else self.state_fc(self.initialPrior[label])
            )

        rel_input = torch.zeros([bbox_num * 2, frame_num - 1, features.size(1)]).to(
            features.device
        )  # (2 * bbox_num, frame_num - 1, feature_dim)
        rel_prior = torch.zeros([bbox_num * 2, frame_num - 1, features.size(1)]).to(
            features.device
        )  # (2 * bbox_num, frame_num - 1, feature_dim)
        position_embed = torch.zeros(
            [bbox_num * 2, frame_num - 1, features.size(1)]
        ).to(features.device)
        ...
        for frame_idx in range(frame_num - 1):
            rel_input[
                : torch.sum((im_idx == frame_idx) + (im_idx == frame_idx + 1)),
                frame_idx,
                :,
            ] = features[(im_idx == frame_idx) + (im_idx == frame_idx + 1)]

            position_embed[: torch.sum(im_idx == frame_idx), frame_idx, :] = (
                self.position_embedding.weight[0]
            )
            position_embed[
                torch.sum(im_idx == frame_idx) : torch.sum(im_idx == frame_idx)
                + torch.sum(im_idx == frame_idx + 1),
                frame_idx,
                :,
            ] = self.position_embedding.weight[1]

            if self.use_temporal_prior:
                rel_prior[: torch.sum(im_idx == frame_idx), frame_idx, :] = (
                    specificPrior[im_idx == frame_idx]
                    if frame_idx != 0
                    else initialPrior[im_idx == frame_idx]
                )
                rel_prior[
                    torch.sum(im_idx == frame_idx) : torch.sum(
                        (im_idx == frame_idx) + (im_idx == frame_idx + 1)
                    ),
                    frame_idx,
                    :,
                ] = initialPrior[im_idx == frame_idx + 1]

            idx[
                : torch.sum((im_idx == frame_idx) + (im_idx == frame_idx + 1)),
                frame_idx,
            ] = im_idx[(im_idx == frame_idx) + (im_idx == frame_idx + 1)]

        mask = (
            (torch.sum(rel_input.view(-1, features.size(1)), dim=1) == 0)
            .view(bbox_num * 2, frame_num - 1)
            .permute(1, 0)
        )

        rel_output, attention_weights = self.decoder(
            rel_input, rel_prior, mask, position_embed
        )

        temporal_features = torch.zeros_like(features).to(features.device)
        for frame_idx in range(frame_num - 1):
            if frame_idx == 0:
                temporal_features[im_idx == frame_idx] = rel_output[:, frame_idx][
                    idx[:, frame_idx] == frame_idx
                ]

            temporal_features[im_idx == frame_idx + 1] = rel_output[:, frame_idx][
                idx[:, frame_idx] == frame_idx + 1
            ]
...
class ensemble_decoder(nn.Module):
    ...
    def forward(
        ...
        features = torch.zeros(
            [spatial_features.size(0), self.window_size, spatial_features.size(1)]
        ).to(spatial_features.device)
        priors = torch.zeros(
            [spatial_features.size(0), self.window_size, spatial_features.size(1)]
        ).to(spatial_features.device)
        masks = torch.zeros(
            [spatial_features.size(0), self.window_size], dtype=torch.bool
        ).to(spatial_features.device)
        position_embed = torch.zeros(
            [spatial_features.size(0), self.window_size, spatial_features.size(1)]
        ).to(spatial_features.device)
        ...
        for rel_idx in range(spatial_features.size(0)):
            ...
            if label not in history_anno_dict.keys():
                history_anno_dict[label] = [None for _ in range(frame_num)]
                history_score_dict[label] = torch.zeros([frame_num]).to(
                    spatial_features.device
                )
                history_feature_dict[label] = torch.zeros(
                    [frame_num, spatial_features.size(1)]
                ).to(spatial_features.device)
            ...
            if self.training:
                history_score_dict[label][frame_idx] = 1.0
                history_feature_dict[label][frame_idx] = (
                    (spatial_features[rel_idx] + temporal_features[rel_idx]) / 2
                )  # torch.cat((spatial_features[rel_idx], temporal_features[rel_idx]))
            ...
            masks[rel_idx, len(candidate_idxes) :] = True
        ...
        output, attention_weights = self.decoder(
            ...
            priors.permute(1, 0, 2),
            masks,
            position_embed.permute(1, 0, 2),
        )

        ensemble_features = torch.zeros(
            (spatial_features.size(0), spatial_features.size(1))
        ).to(spatial_features.device)
        for rel_idx in range(spatial_features.size(0)):
            ensemble_features[rel_idx] = output[
                int(self.window_size - sum(masks[rel_idx]) - 1), rel_idx
            ]
