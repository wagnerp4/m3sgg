...
class ObjectClassifier(nn.Module):
    ...
    def __init__(self, mode="edgecls", obj_classes=None):
        ...
        embed_vecs = obj_edge_vectors(
            self.obj_classes, wv_type="glove.6B", wv_dir="data", wv_dim=200
        )
    ...
    def forward(self, entry):
        if self.mode == "edgecls":
            ...
            entry["distribution"] = torch.nn.functional.one_hot(
                entry["pred_labels"], len(self.obj_classes)
            ).to(torch.float32)
        else:
...
class ActionClassifier(nn.Module):
    def __init__(self, mode="edgecls", verb_classes=None):
        ...
        # Verb features have dimension 2048
        self.decoder_lin = nn.Sequential(
            nn.Linear(2048, 1024),
            nn.BatchNorm1d(1024),
            nn.ReLU(),
    ...
    def forward(self, entry):
        if self.mode != "easgcls":
            ...
            entry["distribution_verb"] = torch.nn.functional.one_hot(
                entry["pred_labels_verb"], len(self.verb_classes)
            ).to(torch.float32)
        else:
            if self.training:
                ...
                entry["distribution_verb"] = torch.softmax(
                    entry["distribution_verb"], dim=1
                )
                entry["pred_labels_verb"] = torch.max(
                    entry["distribution_verb"], dim=1
                )[1]
...
class STTran(nn.Module):
    def __init__(
        ...
        self.action_classifier = ActionClassifier(
            mode=self.mode, verb_classes=self.verb_classes
        )
        ...
        self.verb_fc = nn.Linear(2048, 512)
        ...
        embed_vecs = verb_edge_vectors(
            self.verb_classes, wv_type="glove.6B", wv_dir="data", wv_dim=200
        )
        # embed_vecs = obj_edge_vectors(self.verb_classes, wv_type='glove.6B', wv_dir='data', wv_dim=200)
