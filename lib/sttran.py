    ...
    def forward(self, entry):
        if self.mode == "predcls":
            ...
            if self.training:
                ...
                global_idx = torch.arange(0, entry["boxes"].shape[0]).to(box_idx.device)
            ...
            if self.training:
                ...
                obj_features = torch.cat(
                    (entry["features"], obj_embed, pos_embed), 1
                ...
                for i in range(b):
                    ...
                    for j in range(len(self.classes) - 1):
                        ...
                        if inds.numel() > 0:
                            ...
                            keep = nms(
                                cls_boxes[order, :], cls_scores[order], 0.3
                            )  # Lowered from 0.6 to 0.3 for better suppression of overlapping boxes
                ...
                if len(final_boxes) > 0:
                    ...
                    # Create empty tensors with proper shapes
                    device = entry["boxes"].device
                    entry["boxes"] = torch.empty((0, 5), device=device)
                    entry["distribution"] = torch.empty(
                        (0, len(self.classes)), device=device
                    )
                    entry["features"] = torch.empty(
                        (0, entry["features"].shape[1]), device=device
                    )
                    entry["pred_labels"] = torch.empty(
                        (0,), dtype=torch.long, device=device
                    )
                    entry["pred_scores"] = torch.empty((0,), device=device)
                    return entry
...
class STTran(nn.Module):
    ...
    def forward(self, entry):
        ...
        global_output, global_attention_weights, local_attention_weights = (
        ...
        entry["contact_distribution"] = self.c_rel_compress(global_output)

        entry["spatial_distribution"] = torch.sigmoid(entry["spatial_distribution"])
        entry["contact_distribution"] = torch.sigmoid(entry["contact_distribution"])

        # Add final outputs for visualization and compatibility
        entry["final_boxes"] = entry["boxes"][:, 1:]  # Remove batch index column
        entry["final_labels"] = (
            entry["pred_labels"]
            if "pred_labels" in entry
            else torch.tensor([], device=entry["boxes"].device)
        )
...
class STKET(nn.Module):
    def __init__(
        ...
        print(
            "[STTran] Mode: {}, Pred_contact_threshold: {:.1f}, window_size: {:d}".format(
