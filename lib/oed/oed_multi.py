...
class OEDMulti(nn.Module):
    ...
    def __init__(self, conf, dataset):
        ...
        self.contacting_class_embed = nn.Linear(
            self.hidden_dim, self.num_contacting_classes
        )
        ...
        self.feature_proj = nn.Linear(
            2048, self.hidden_dim
        )  # Project from object detector features to hidden dim
        ...
        if self.rel_temporal_embed_head:
            ...
            self.temporal_contacting_class_embed = copy.deepcopy(
                self.contacting_class_embed
            )
    ...
    def forward(self, entry, targets=None):
        ...
        mask = torch.zeros(
            batch_size, num_objects, dtype=torch.bool, device=features.device
        )
        ...
        hopd_out, interaction_decoder_out = self.transformer(
            src, mask, self.query_embed.weight, pos, embed_dict, targets, cur_idx=0
        )[:2]
        ...
        if self.rel_temporal_embed_head:
            ...
            outputs_spatial_class = self.temporal_spatial_class_embed(
                interaction_decoder_out
            )
            outputs_contacting_class = self.temporal_contacting_class_embed(
                interaction_decoder_out
            )
        else:
            ...
            outputs_contacting_class = self.contacting_class_embed(
                interaction_decoder_out
            )
        ...
        outputs_attn_class = outputs_attn_class.view(
            -1, outputs_attn_class.size(-1)
        )  # (B*N, num_attn_classes)
        outputs_spatial_class = outputs_spatial_class.view(
            -1, outputs_spatial_class.size(-1)
        )  # (B*N, num_spatial_classes)
        outputs_contacting_class = outputs_contacting_class.view(
            -1, outputs_contacting_class.size(-1)
        )  # (B*N, num_contacting_classes)
        ...
        if isinstance(attention_gt, list) and len(attention_gt) > 0:
            attention_gt = torch.tensor(
                attention_gt, dtype=torch.long, device=outputs_attn_class.device
            )
        if isinstance(spatial_gt, list) and len(spatial_gt) > 0:
            spatial_gt = [
                torch.tensor(sg, dtype=torch.long, device=outputs_spatial_class.device)
                for sg in spatial_gt
            ]
        if isinstance(contact_gt, list) and len(contact_gt) > 0:
            contact_gt = [
                ...
                for cg in contact_gt
            ]
        if isinstance(labels, list) and len(labels) > 0:
            labels = torch.tensor(
                labels, dtype=torch.long, device=outputs_obj_class.device
            )
        ...
        if actual_batch_size > model_output_size:
            ...
            logger.warning(
                f"Ground truth batch size ({actual_batch_size}) exceeds model outputs ({model_output_size}). Truncating ground truth."
            )
            if isinstance(attention_gt, torch.Tensor):
        ...
        pair_idx = torch.stack(
            ...
            dim=1,
        ).to(device=outputs_obj_class.device)
        ...
        if "im_idx" in entry and len(entry["im_idx"]) >= actual_batch_size:
            ...
            im_idx = torch.zeros(
                actual_batch_size, dtype=torch.long, device=outputs_obj_class.device
            )
        ...
        out = {
            ...
            "attention_distribution": torch.softmax(
                outputs_attn_class, dim=1
            ),  # Evaluator expects softmax
            "spatial_distribution": torch.softmax(
                outputs_spatial_class, dim=1
            ),  # Evaluator expects softmax
            "contact_distribution": torch.softmax(
                outputs_contacting_class, dim=1
            ),  # Evaluator expects softmax
            # Ground truth labels
            ...
            "pred_scores": torch.ones(
                actual_batch_size, device=outputs_obj_class.device
            ),  # Default confidence scores
            "scores": torch.ones(
                actual_batch_size, device=outputs_obj_class.device
            ),  # Object confidence scores
            "pred_labels": torch.argmax(
                outputs_obj_class, dim=1
        ...
        if "boxes" in entry:
            ...
            batch_indices = torch.zeros(
                actual_batch_size, 1, device=outputs_obj_coord.device
            )
