...
class OEDSingle(nn.Module):
    ...
    def forward(self, entry, targets=None):
        ...
        src = features.unsqueeze(
            0
        )  # Add batch dimension: (1, num_objects, feature_dim)
        mask = torch.zeros(
        ...
        hopd_out, interaction_decoder_out, ins_attn_weight, rel_attn_weight = (
            self.transformer(src, mask, self.query_embed.weight, pos)[:4]
        )
        ...
        pair_idx = torch.stack(
            ...
            dim=1,
        ).to(device=outputs_obj_class[-1].device)
        ...
        if "im_idx" in entry and len(entry["im_idx"]) >= num_pairs:
            ...
            im_idx = torch.zeros(
                num_pairs, dtype=torch.long, device=outputs_obj_class[-1].device
            )
        ...
        out = {
            ...
            "pred_scores": torch.ones(
                self.num_queries, device=outputs_obj_class[-1].device
            ),  # Default confidence scores
            "scores": torch.ones(
                self.num_queries, device=outputs_obj_class[-1].device
            ),  # Object confidence scores
            "pred_labels": torch.argmax(
                outputs_obj_class[-1], dim=1
        ...
        if "boxes" in entry:
            ...
            batch_indices = torch.zeros(
                self.num_queries, 1, device=outputs_obj_class[-1].device
            )
