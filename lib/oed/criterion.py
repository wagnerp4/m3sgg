...
class SetCriterionOED(nn.Module):
    ...
    def __init__(self, num_obj_classes, num_queries, matcher, weight_dict, eos_coef, losses, conf):
    ...
    def loss_obj_labels(self, outputs, targets, indices, num_interactions, log=True):
        ...
        target_classes = torch.full(
            src_logits.shape[:2], self.num_obj_classes,
    ...
    def loss_sub_obj_boxes(self, outputs, targets, indices, num_interactions):
        ...
        if src_sub_boxes.shape[0] == 0:
            ...
            loss_sub_giou = 1 - torch.diag(generalized_box_iou(
                box_cxcywh_to_xyxy(src_sub_boxes),
                box_cxcywh_to_xyxy(target_sub_boxes)
            ))
            loss_obj_giou = 1 - torch.diag(generalized_box_iou(
