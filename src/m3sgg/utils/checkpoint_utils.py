...
def check_disk_space_and_configure_checkpointing(
    save_path: str, logger: logging.Logger, conf
) -> Tuple[bool, str]:
    """
    ...
    logger.info(
        f"Disk space check - Total: {total_gb:.1f}GB, Used: {used_gb:.1f}GB, Free: {free_gb:.1f}GB"
    )
    ...
    if free_gb < required_space_gb:
        ...
        logger.warning(
            f"Required: {required_space_gb:.2f}GB, Available: {free_gb:.1f}GB"
        )
        ...
        logger.warning(
            f"Available: {free_gb:.1f}GB, Recommended: {required_space_gb * 2:.2f}GB"
        )
        logger.warning(
            "Using conservative checkpoint strategy - only saving best model."
        )
...
def safe_save_checkpoint(
    ...
    try:
        ...
        if free_gb < required_space_gb:
            if logger:
                logger.error(
                    f"Insufficient disk space to save checkpoint: {free_gb:.2f}GB available, {required_space_gb:.2f}GB required"
                )
            return False
...
def validate_checkpoint_file(
    checkpoint_path: str, logger: Optional[logging.Logger] = None
) -> bool:
    """
    ...
    try:
        ...
        if file_size_mb < 10:  # Less than 10MB is suspicious
            if logger:
                logger.warning(
                    f"Checkpoint file is unusually small: {file_size_mb:.1f}MB"
                )
        ...
        if logger:
            logger.info(
                f"Checkpoint validation successful: {checkpoint_path} ({file_size_mb:.1f}MB)"
            )
        ...
        if logger:
            logger.error(
                f"Checkpoint validation failed for {checkpoint_path}: {str(e)}"
            )
        return False
