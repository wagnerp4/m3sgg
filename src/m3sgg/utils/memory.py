...
from m3sgg.utils.uncertainty import normalize_batch_uncertainty
...
def memory_computation(
    ...
    for i in unc_list_rel.keys():
        rel_features = torch.tensor(
            np.load(rel_emb_path + str(i) + ".npy", allow_pickle=True)
        )

        if not obj_all_u and obj_mem:
            obj_features = torch.tensor(
                np.load(obj_emb_path + str(i) + ".npy", allow_pickle=True)
            )
            ...
            for k in unq_batch_classes:
                ...
                obj_norm_factor[k] = obj_norm_factor[k] + torch.sum(
        ...
        if not rel_all_u:
            for rel in rel_class_num.keys():
                ...
                for k in unq_batch_classes:
                    unq_idx = torch.where(batch_classes == k)[0]
                    rel_norm_factor[rel][k] = rel_norm_factor[rel][k] + torch.sum(
                        batch_unc[index[unq_idx], batch_classes[unq_idx]]
                    )
                # for idx,k in zip(index,batch_classes) :
            ...
            unc_list_rel[i], unc_list_obj[i] = normalize_batch_uncertainty(
                unc_list_rel[i],
                cls_rel_uc,
                unc_list_obj[i],
                cls_obj_uc,
                obj_unc=obj_unc,
                background_mem=include_bg_mem,
    ...
    if obj_mem and obj_weight_type == "simple":
        ...
        tmp[nz_idx] = tmp[nz_idx] / (
            obj_norm_factor[nz_idx].unsqueeze(-1).repeat(1, obj_feature_dim)
        )
        obj_memory = tmp
    ...
    if rel_weight_type == "simple":
        for rel in rel_memory.keys():
            ...
            tmp[nz_idx] = tmp[nz_idx] / (
                rel_norm_factor[rel][nz_idx].unsqueeze(-1).repeat(1, rel_feature_dim)
            )
            rel_memory[rel] = tmp
