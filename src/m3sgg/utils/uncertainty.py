...
from m3sgg.utils.ds_track import get_sequence
...
class uncertainty_values:
    ...
    def __init__(
        ...
        cls_rel_uc = {"attention": {}, "spatial": {}, "contacting": {}}
    ...
    def stats(self):
        for k in self.cls_obj_uc.keys():
            if len(self.cls_obj_uc[k]["al"]) > 0:
                self.cls_obj_uc[k]["al"] = (
                    mean(self.cls_obj_uc[k]["al"]),
                    pstdev(self.cls_obj_uc[k]["al"]),
                )
                self.cls_obj_uc[k]["ep"] = (
                    mean(self.cls_obj_uc[k]["ep"]),
                    pstdev(self.cls_obj_uc[k]["ep"]),
                )
            ...
            for cls in self.cls_rel_uc[rel].keys():
                if len(self.cls_rel_uc[rel][cls]["al"]) > 0:
                    self.cls_rel_uc[rel][cls]["al"] = (
                        mean(self.cls_rel_uc[rel][cls]["al"]),
                        pstdev(self.cls_rel_uc[rel][cls]["al"]),
                    )
                    self.cls_rel_uc[rel][cls]["ep"] = (
                        mean(self.cls_rel_uc[rel][cls]["ep"]),
                        pstdev(self.cls_rel_uc[rel][cls]["ep"]),
                    )

    def stats2(self):
        for k in self.cls_obj_uc.keys():
            if len(self.cls_obj_uc[k]["al"]) > 0:
                self.cls_obj_uc[k]["both"] = sum(
                    np.exp(self.cls_obj_uc[k]["al"] + self.cls_obj_uc[k]["ep"])
                )
        ...
        for rel in self.cls_rel_uc.keys():
            for cls in self.cls_rel_uc[rel].keys():
                if len(self.cls_rel_uc[rel][cls]["al"]) > 0:
                    ...
                    self.cls_rel_uc[rel][cls]["al"] = sum(
                        np.exp(self.cls_rel_uc[rel][cls]["al"])
                    )
                    self.cls_rel_uc[rel][cls]["ep"] = sum(
                        np.exp(self.cls_rel_uc[rel][cls]["ep"])
                    )


def uncertainty_computation(
    data,
    dataset,
    object_detector,
    model,
    ...
    with torch.no_grad():
        ...
        if obj_mem:
            ...
            np.save(
                obj_emb_path + str(index) + ".npy",
                obj_features.detach().cpu().numpy(),
                allow_pickle=True,
            )
        ...
        if obj_unc:
            ...
            for u in ["al", "ep"]:
                ...
                obj_cls_uc = pred["obj_" + u + "_uc"][
                    torch.arange(pred["obj_" + u + "_uc"].size(0)), obj_labels
                ].cpu()

                # print(obj_cls_uc.shape)
                batch_unc = torch.zeros(
                    obj_labels.shape[0], pred["obj_" + u + "_uc"].size(1)
                )
                ...
                for cls in unq_labels:
                    if not background_mem and cls == 0:
                        continue
                    unc_vals.cls_obj_uc[cls.item()][u] += obj_cls_uc[
                        torch.where(obj_labels == cls)[0]
                    ].tolist()
        ...
        np.save(
            rel_emb_path + str(index) + ".npy",
            pred["rel_features"].detach().cpu().numpy(),
            allow_pickle=True,
        )
        rel_labels = {
            "attention": pred["attention_gt"],
        ...
        for rel, label in rel_labels.items():
            np.save(
                rel_emb_path + str(index) + "_" + rel + "_labels.npy",
                label,
                allow_pickle=True,
            )
            ...
            if rel_unc:
                for u in ["al", "ep"]:
                    ...
                    for j, l in enumerate(labels):
                        for k in l:
                            unc_vals.cls_rel_uc[rel][k][u].append(
                                pred_rel_unc[j, k].item()
                            )
            else:
                batch_unc = torch.zeros(
                    len(rel_labels[rel]), len(list(unc_vals.cls_rel_uc[rel].keys()))
                )
...
def normalize_batch_uncertainty(
    unc_list_rel,
    cls_rel_uc,
    unc_list_obj,
    cls_obj_uc,
        ...
        if obj_unc:
            ...
            for k in unq_batch_classes:
                ...
                batch_unc[index[unq_idx], batch_classes[unq_idx]] = np.exp(
                    batch_unc[index[unq_idx], batch_classes[unq_idx]]
                ) / (cls_obj_uc[cls][u] + 1e-12)
