...
class BaseLLMWrapper(ABC):
    ...
    def chat_about_scene_graph(
        self, scene_graph_data: Dict[str, Any], user_question: str, **kwargs
    ) -> str:
    ...
    def _format_scene_graph(self, scene_graph_data: Dict[str, Any]) -> str:
        ...
        if triples:
            try:
                ...
                return (
                    " ".join(sentences)
                    if sentences
                    else "No objects or relationships detected in the scene."
                )
            except Exception:
                pass
        ...
        if objects:
            ...
            if unique_objects:
                description_parts.append(
                    f"Detected objects: {', '.join(unique_objects)}"
                )
        ...
        if relationships:
            ...
            if rel_descriptions:
                description_parts.append(
                    f"Relationships: {'; '.join(rel_descriptions)}"
                )
        ...
        if avg_objects:
            ...
            description_parts.append(
                f"Average {avg_count:.1f} objects per frame across {total_frames} frames"
            )

        return (
            ". ".join(description_parts)
            if description_parts
            else "No scene graph data available"
        )

    def _create_conversation_prompt(
        self, scene_description: str, user_question: str
    ) -> str:
...
class GemmaLLMWrapper(BaseLLMWrapper):
    ...
    def _clean_response(self, response: str) -> str:
        ...
        response = re.sub(
            r"^(Assistant:|User:|Human:)\s*", "", response, flags=re.IGNORECASE
        )
    ...
    def get_response(self, user_question: str, **kwargs) -> str:
        ...
        if self.current_scene_graph:
            response = self.llm_wrapper.chat_about_scene_graph(
                self.current_scene_graph, user_question, **kwargs
            )
        else:
...
class SceneGraphFormatter:
    ...
    def format_scene_graph(self, scene_graph_data: Dict[str, Any]) -> str:
        ...
        if "total_frames" in scene_graph_data:
            ...
            descriptions.append(
                f"Video has {total_frames} total frames, analyzed {processed_frames} frames"
            )
        ...
        return (
            ". ".join(descriptions) if descriptions else "No scene graph data available"
        )
    ...
    def _format_frame_objects(self, frame_objects: List[List[Dict[str, Any]]]) -> str:
        ...
        for obj_name in sorted(all_objects):
            ...
            obj_descriptions.append(
                f"{obj_name} (avg confidence: {avg_confidence:.2f}, present in {presence_frames} frames)"
            )

        return f"Detected objects: {', '.join(obj_descriptions)}"

    def _format_frame_relationships(
        self, frame_relationships: List[List[Dict[str, Any]]]
    ) -> str:
        ...
        for rel_type, rels in rel_by_type.items():
            ...
            rel_descriptions.append(
                f"{rel_type} ({count} instances, avg confidence: {avg_confidence:.2f})"
            )
    ...
    def _format_temporal_analysis(self, scene_graph_data: Dict[str, Any]) -> str:
        ...
        avg_relationships = (
            sum(relationships) / len(relationships) if relationships else 0
        )
        avg_confidence = sum(confidences) / len(confidences) if confidences else 0

        temporal_desc = (
            f"Temporal analysis: average {avg_objects:.1f} objects per frame"
        )
...
def create_llm_wrapper(
    model_name: str = "google/gemma-3-270m", model_type: str = "gemma", **kwargs
) -> BaseLLMWrapper:
