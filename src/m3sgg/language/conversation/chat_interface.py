...
from ..language_modeling.llm import create_conversation_manager, SceneGraphFormatter
...
class SceneGraphChatInterface:
    ...
    def _show_scene_graph_context(self) -> None:
        ...
        if st.session_state.scene_graph_context:
            with st.expander("Current Scene Graph Context", expanded=True):
                ...
                st.text_area(
                    "Scene Description", context_text, height=200, disabled=True
                )
        else:
            st.info(
                "No scene graph context available. Upload a video to generate scene graphs."
            )
    ...
    def get_conversation_stats(self) -> Dict[str, Any]:
        ...
        user_messages = sum(
            1 for msg in st.session_state.chat_messages if msg["is_user"]
        )
    ...
    def add_system_message(self, message: str) -> None:
        ...
        """
        st.session_state.chat_messages.append(
            {"message": f"[System] {message}", "is_user": False}
        )
