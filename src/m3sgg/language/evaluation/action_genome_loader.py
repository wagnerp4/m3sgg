...
class ActionGenomeLoader:
    ...
    def __init__(self, data_path: str, split: str = "val", max_videos: Optional[int] = None):
        ...
        logger.info(f"Loaded ActionGenome {split} split with {len(self.annotations)} videos")
    ...
    def _load_annotations(self) -> Dict[str, Any]:
        ...
        object_bbox_path = self.data_path / "annotations" / "object_bbox_and_relationship.pkl"
        ...
        logger.info(f"Filtered to {len(filtered_annotations)} videos with sufficient frames")
        return filtered_annotations
    
    def _process_frame_annotation(self, frame_key: str, frame_data: List[Dict], 
                                person_data: Dict) -> Optional[Dict[str, Any]]:
        ...
        for obj_data in frame_data:
            ...
            if isinstance(obj_data["class"], str):
                ...
                class_name = self.object_classes[obj_data["class"]] if obj_data["class"] < len(self.object_classes) else "unknown"
            
            obj = {
                "object_id": f"obj_{len(frame_annotation['objects'])}",
                "class": class_name,
                "bbox": obj_data["bbox"].tolist() if hasattr(obj_data["bbox"], 'tolist') else obj_data["bbox"],
                "visible": True
            }
        ...
        if "attention_relationship" in obj_data:
            for rel_idx in obj_data["attention_relationship"]:
                if isinstance(rel_idx, str):
                    predicate = rel_idx
                elif isinstance(rel_idx, int) and rel_idx < len(self.relationship_classes):
    ...
    def _generate_caption_from_triples(self, triples: List[Tuple[str, str, str]]) -> str:
        ...
        if relationship_counts:
            sorted_rels = sorted(relationship_counts.items(), key=lambda x: x[1], reverse=True)
            top_rels = [rel for rel, count in sorted_rels[:3]]
            if top_rels:
                parts.append(f"Activities: {', '.join(top_rels)}")
        
        return ". ".join(parts) if parts else "A scene with various objects and activities."
