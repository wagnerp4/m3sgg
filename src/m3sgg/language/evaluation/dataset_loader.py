...
try:
    ...
    DATASETS_AVAILABLE = False
    logging.warning(
        f"huggingface datasets library not available: {e}. Install with: pip install datasets"
    )
...
class MSRVTTLoader:
        ...
        for ds_id in CANDIDATE_MSRVTT_DATASETS:
            try:
                ...
                for k in list(split_map.keys()):
                    try:
                        split_map[k] = split_map[k].map(
                            _normalize,
                            remove_columns=[
                                c for c in split_map[k].column_names if c not in []
                            ],
                        )
                    except Exception:
                        # Fallback: build a list of dicts
                        split_map[k] = [_normalize(rec) for rec in split_map[k]]
    ...
    def create_subset(
        ...
        train_indices = random.sample(
            range(len(train_data)), min(train_size, len(train_data))
        )
        test_indices = random.sample(
            range(len(test_data)), min(test_size, len(test_data))
        )
        ...
        logger.info(
            f"Created subset: {len(train_subset)} train, {len(test_subset)} test samples"
        )

        return subset

    def _save_subset_metadata(
        self, subset: Dict, train_size: int, test_size: int, random_seed: int
    ):
    ...
    def get_sample_info(
        self, subset: Dict, split: str = "test", sample_idx: int = 0
    ) -> Dict:
