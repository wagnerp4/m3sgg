...
class ActionGenomeSummarizationBenchmark:
    ...
    def load_dataset(self, max_videos: Optional[int] = None):
        ...
        logger.info(f"Dataset loaded with {len(self.dataset_loader.get_video_list())} videos")
    ...
    def run_benchmark(self, subset_size: int = 100, 
                     models: Optional[List[str]] = None,
                     linearizers: Optional[List[str]] = None) -> Dict[str, Any]:
        ...
        if not subset["test"] or len(subset["test"]) == 0:
            logger.error("No data available for benchmark. Check ActionGenome dataset loading.")
        ...
        for linearizer in linearizers:
            ...
            for model_name in models:
                ...
                for i, sample in enumerate(subset["test"]):
                    ...
                    scene_graph_text = " ".join(sentences) if sentences else "No objects or relationships detected."
                ...
                logger.info(f"Completed evaluation for {model_name} with {linearizer} linearizer")
