...
class VLMSceneGraphGenerator(nn.Module):    
    ...
    def _create_object_classifier(self):
        ...
        return nn.Sequential(
            nn.Linear(2048, 512),
            nn.ReLU(),
            nn.Linear(512, len(self.obj_classes))
        )
    ...
    def _create_prompt(self, image_description: str, objects: List[str], 
    ...
    def _convert_relationships_to_distributions(self, relationships: List[Tuple[str, str, str]], 
                                             pair_idx: torch.Tensor, 
                                             pred_labels: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        attention_dist = torch.zeros(num_pairs, self.attention_class_num, device=device, requires_grad=False)
        spatial_dist = torch.zeros(num_pairs, self.spatial_class_num, device=device, requires_grad=False) 
        contact_dist = torch.zeros(num_pairs, self.contact_class_num, device=device, requires_grad=False)
        ...
        for subj, rel, obj in relationships:
            ...
            for i, (subj_idx, obj_idx) in enumerate(pair_idx):
                ...
                if matched_subj == subj_name and matched_obj == obj_name:
                    ...
                    matched_rel = fuzzy_match_relationship(rel, self.attention_relationships)
                    if matched_rel:
                    ...
                    matched_rel = fuzzy_match_relationship(rel, self.spatial_relationships)
                    if matched_rel:
                    ...
                    matched_rel = fuzzy_match_relationship(rel, self.contact_relationships)
                    if matched_rel:
    ...
    def _crop_objects_from_image(self, image: torch.Tensor, boxes: torch.Tensor, 
                                im_info: torch.Tensor) -> List[Image.Image]:
    ...
    def forward(self, entry: Dict, im_data: torch.Tensor = None) -> Dict:
        ...
        if self.model_type == "causal_lm":
            ...
            text_inputs = self.tokenizer.tokenizer(prompt, return_tensors="pt", padding=True, truncation=True)
        ...
        if self.model_type == "causal_lm":
            ...
            if hasattr(self.model, 'get_vision_tower'):
                ...
                print(f"Warning: Model {self.model_name} doesn't have vision tower, using fallback image processing")
                pixel_values = torch.randn(1, 3, 224, 224).to(self.device, dtype=self.model.dtype)
        elif self.model_type == "blip":
            # BLIP models use the processor directly for images only
            pixel_values = self.tokenizer.image_processor(images=image_pil, return_tensors="pt").pixel_values.to(self.device)
        else:
            raise ValueError(f"Unknown model type: {self.model_type}")
        ...
        output = {
            ...
            "boxes": entry.get("boxes", torch.zeros((len(pred_labels), 5), device=self.device)),  # Required for evaluation
            "scores": entry.get("scores", torch.ones(len(pred_labels), device=self.device)),  # Required for evaluation
            "pred_scores": torch.softmax(obj_distribution, dim=1).max(dim=1)[0],  # Use actual confidence scores
