...
def get_args_parser():
    ...
    parser.add_argument(
        "--freeze_mode", default=0, type=int
    )  # freeze parameters other than those of temporal module and head
    ...
    parser.add_argument(
        "--temporal_feature_encoder", action="store_true", default=False
    )
    parser.add_argument(
        "--instance_temporal_interaction", action="store_true", default=False
    )
    parser.add_argument(
        "--relation_temporal_interaction", action="store_true", default=False
    )
    parser.add_argument(
        "--query_temporal_interaction", action="store_true", default=False
    )  # concat pair query and relation query together to perform temporal query interaction
    parser.add_argument(
        "--temporal_embed_head", action="store_true", default=False
    )  # use temporal head and keep spatial head frozen
    parser.add_argument(
        "--temporal_decoder_init", action="store_true", default=False
    )  # use spatial decoder weight to initialize the weight of temporal decoder
    parser.add_argument(
        "--one_dec", action="store_true", default=False
    )  # only use one decoder, view predcls as a predicate classification problem
    parser.add_argument(
        "--use_roi", action="store_true", default=False
    )  # use RoI feature to initialize query feature
    parser.add_argument(
        "--one_temp", action="store_true", default=False
    )  # only use one temporal query interaction, not in a coarse-to-fine way
    parser.add_argument(
        "--no_update_pair", action="store_true", default=False
    )  # concat query to temporal interaction, but only update relation query
    ...
    parser.add_argument(
        "--p_obj", default=0.7, type=float, help="Reweighting parameter for obj"
    )
    parser.add_argument(
        "--p_rel", default=0.7, type=float, help="Reweighting parameter for verb"
    )
...
def main(args):
    ...
    if args.freeze_mode == 1:
        ...
        for name, p in model_without_ddp.named_parameters():
            if (
                "temporal" in name
                or "semantic_pos" in name
                or (args.aux_learnable_query and "query_embed" in name)
            ):
                if "query_embed" in name:
                    print(name)
                p.requires_grad = True
            elif not args.query_temporal_interaction and (
                (
                    not args.instance_temporal_interaction
                    and ("bbox_embed" in name or "obj_class_embed" in name)
                )
                or (
                    not args.relation_temporal_interaction
                    and "class_embed" in name
                    and "obj_class_embed" not in name
                )
            ):
                # print(name)
                p.requires_grad = True
        ...
        param_dicts = [
            {
                "params": [
                    ...
                    if not name_match_baseline(n) and p.requires_grad
                ]
            },
            {
                "params": [
    ...
    if args.resume:
        ...
        if args.eval:
                    ...
                    if args.temporal_decoder_init:  # load temporal decoder weights
                        if "transformer.decoder.layers.2" in name:
                            replaced_name = name.replace(
                                ...
                                "transformer.ins_temporal_interaction_decoder.layers.0",
                            )
                        if "transformer.interaction_decoder.layers.2" in name:
                            replaced_name = name.replace(
                                "transformer.interaction_decoder.layers.2",
                                "transformer.rel_temporal_interaction_decoder.layers.0",
                            )
                    if args.temporal_embed_head and args.relation_temporal_interaction:
                        if (
                            "class_embed" in name and not "obj_class_embed" in name
                    ...
                    if args.method2:
                        ...
                        if "transformer.decoder.layers.0" in name:
                            replaced_name = name.replace(
                                ...
                                "transformer.ref_decoder.layers.0",
                            )
            ...
            model_without_ddp.load_state_dict(checkpoint["model"], strict=False)
    if args.eval:
        test_stats = evaluate_dsgg(
            args.dataset_file, model, postprocessors, data_loader_val, device, args
        )
...
def name_match_baseline(name):
    baseline_modules_without_head = [
        "transformer.encoder.",
        "transformer.decoder.",
        "transformer.interaction_decoder.",
        "input_proj.",
        "query_embed.",
        "backbone.",
    ]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        "OED training and evaluation script", parents=[get_args_parser()]
    )
