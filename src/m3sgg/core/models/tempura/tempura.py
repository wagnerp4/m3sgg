...
from m3sgg.core.models.tempura.gmm_heads import GMM_head
from m3sgg.core.models.tempura.transformer_tempura import transformer
...
class ObjectClassifier(nn.Module):
    ...
    def __init__(
        ...
        if self.tracking:
            ...
            encoder_layer = EncoderLayer(
                d_model=d_model, dim_feedforward=1024, nhead=8, batch_first=True
            )
            self.positional_encoder = PositionalEncoding(
                d_model, 0.1, 600 if mode == "sgdet" else 400
            )
        ...
        if obj_head == "gmm":
            ...
            self.decoder_lin = GMM_head(
                hid_dim=1024, num_classes=len(self.classes), rel_type=None, k=self.GMM_K
            )
    ...
    def clean_class(self, entry, b, class_idx):
        ...
        for i in range(b):
            ...
            new_box = pred_boxes[
                entry["pred_labels"][entry["boxes"][:, 0] == i] == class_idx
            ]
            new_feats = feats[
                entry["pred_labels"][entry["boxes"][:, 0] == i] == class_idx
            ]
            new_mem_feats = mem_feats[
                entry["pred_labels"][entry["boxes"][:, 0] == i] == class_idx
            ]
            new_scores = scores[
                entry["pred_labels"][entry["boxes"][:, 0] == i] == class_idx
    ...
    def classify(self, entry, obj_features, phase="train", unc=False):
        if self.tracking:
            ...
            if len(indices) > 1:
                ...
                for index in indices[1:]:
                    im_idx, counts = torch.unique(
                        entry["boxes"][index][:, 0].view(-1),
                        return_counts=True,
                        sorted=True,
                    )
                    ...
                    pos = torch.cat(
                        [
                            torch.LongTensor([im] * count)
                            for im, count in zip(range(len(counts)), counts)
                        ]
                    )
                    pos_index.append(pos)
                sequence_features = pad_sequence(
                    [obj_features[index] for index in indices[1:]], batch_first=True
                )
                masks = (
                    1
                    - pad_sequence(
                        [torch.ones(len(index)) for index in indices[1:]],
                        batch_first=True,
                    )
                ).bool()
                pos_index = pad_sequence(pos_index, batch_first=True)
                obj_ = self.encoder_tran(
                    self.positional_encoder(sequence_features, pos_index),
                    src_key_padding_mask=masks.cuda(),
                )
                obj_flat = torch.cat(
                    [obj[: len(index)] for index, obj in zip(indices[1:], obj_)]
                )
                indices_flat = (
                    torch.cat(indices[1:]).unsqueeze(1).repeat(1, obj_features.shape[1])
                )
                final_features.scatter_(0, indices_flat, obj_flat)
            if len(indices[0]) > 0:
                non_ = self.encoder_tran(
                    self.positional_encoder(obj_features[indices[0]].unsqueeze(1))
                )
                final_features.scatter_(
                    0,
                    indices[0].unsqueeze(1).repeat(1, obj_features.shape[1]),
                    non_[:, 0, :],
                )
            ...
            if self.mem_compute:
                obj_features = self.memory_hallucinator(
                    memory=self.obj_memory, feat=obj_features
                )
        ...
        if phase == "train":
            if self.obj_head == "gmm":
                if not unc:
                    entry["distribution"] = self.decoder_lin(
                        obj_features, phase=phase, unc=unc
                    )
                else:
                    entry["distribution"] = self.decoder_lin(
                        obj_features, phase="test", unc=False
                    )
                    entry["obj_al_uc"], entry["obj_ep_uc"] = self.decoder_lin(
                        obj_features, unc=unc
                    )
            else:
            ...
            if self.obj_head == "gmm":
                ...
                entry["distribution"] = self.decoder_lin(obj_features)
                entry["distribution"] = torch.softmax(
    ...
    def forward(self, entry, phase="train", unc=False):
        if self.mode == "predcls":
            ...
            if phase == "train":
                ...
                entry["pred_scores"], entry["pred_labels"] = torch.max(
                    entry["distribution"][:, 1:], dim=1
                )
                ...
                for i in range(b):
                    local_human_idx = torch.argmax(
                        entry["distribution"][box_idx == i, 0]
                ...
                entry["pred_scores"][HUMAN_IDX.squeeze()] = entry["distribution"][
                    HUMAN_IDX.squeeze(), 0
                ]

                # drop repeat overlap TODO!!!!!!!!!!!!
                for i in range(b):
                    duplicate_class = torch.mode(
                        entry["pred_labels"][entry["boxes"][:, 0] == i]
                    )[0]
                    ...
                    if (
                        ...
                        duplicate_position = (
                            entry["pred_labels"][present] == duplicate_class
                        )

                        ppp = torch.argsort(
                            entry["distribution"][present][duplicate_position][
                                :, duplicate_class - 1
                            ]
                        )[:-1]
                        for j in ppp:
                            ...
                            entry["pred_labels"][changed_idx] = (
                                torch.argmax(entry["distribution"][changed_idx]) + 1
                            )
                            entry["pred_scores"][changed_idx] = torch.max(
                                entry["distribution"][changed_idx]
                            )
                ...
                union_boxes = torch.cat(
                    (
                        im_idx[:, None],
                        torch.min(
                            entry["boxes"][:, 1:3][pair[:, 0]],
                            entry["boxes"][:, 1:3][pair[:, 1]],
                        ),
                        torch.max(
                            entry["boxes"][:, 3:5][pair[:, 0]],
                            entry["boxes"][:, 3:5][pair[:, 1]],
                        ),
                    ),
                    1,
                )
                ...
                pair_rois = (
                    torch.cat(
                        (
                            entry["boxes"][pair[:, 0], 1:],
                            entry["boxes"][pair[:, 1], 1:],
                        ),
                        1,
                    )
                    .data.cpu()
            ...
            if phase == "train":
                ...
                for i in range(b):
                    ...
                    feats = entry["features"][entry["boxes"][:, 0] == i]
                    if "object_mem_features" in entry.keys():
                        mem_feats = entry["object_mem_features"][
                            entry["boxes"][:, 0] == i
                        ]
                    else:
                    ...
                    for j in range(len(self.classes) - 1):
                        ...
                        if scores.numel() == 0:
                            ...
                            inds = torch.nonzero(torch.argmax(scores, dim=1) == j).view(
                                -1
                            )
                        # if there is det
                        if inds.numel() > 0:
                            ...
                            keep = nms(
                                cls_boxes[order, :], cls_scores[order], 0.6
                            )  # hyperparameter
                ...
                entry["spatial_masks"] = torch.tensor(
...
class TEMPURA(nn.Module):
    def __init__(
        ...
        self.glocal_transformer = transformer(
            ...
            dropout=0.1,
            mode="latter",
            mem_compute=rel_mem_compute,
            mem_fusion=mem_fusion,
            selection=selection,
            selection_lambda=self.selection_lambda,
        )

        if rel_head == "gmm":
            self.a_rel_compress = GMM_head(
                hid_dim=1936,
                num_classes=self.attention_class_num,
                rel_type="attention",
                k=self.GMM_K,
            )
            self.s_rel_compress = GMM_head(
                hid_dim=1936,
                num_classes=self.spatial_class_num,
                rel_type="spatial",
                k=self.GMM_K,
            )
            self.c_rel_compress = GMM_head(
                hid_dim=1936,
                num_classes=self.contact_class_num,
                rel_type="contact",
                k=self.GMM_K,
            )
    ...
    def forward(self, entry, phase="train", unc=False):
        ...
        global_output, rel_features, mem_features, _, _ = self.glocal_transformer(
            features=rel_features, im_idx=entry["im_idx"], memory=self.rel_memory
        )
        ...
        if self.rel_head == "gmm":
            if not unc:
                entry["attention_distribution"] = self.a_rel_compress(
                    global_output, phase, unc
                )
                entry["spatial_distribution"] = self.s_rel_compress(
                    global_output, phase, unc
                )
                entry["contacting_distribution"] = self.c_rel_compress(
                    global_output, phase, unc
                )
            else:
                entry["attention_al_uc"], entry["attention_ep_uc"] = (
                    self.a_rel_compress(global_output, phase, unc)
                )
                entry["spatial_al_uc"], entry["spatial_ep_uc"] = self.s_rel_compress(
                    global_output, phase, unc
                )
                entry["contacting_al_uc"], entry["contacting_ep_uc"] = (
                    self.c_rel_compress(global_output, phase, unc)
                )
        else:
            entry["attention_distribution"] = self.a_rel_compress(global_output)
            if phase == "test":
                entry["attention_distribution"] = entry[
            ...
            entry["contacting_distribution"] = torch.sigmoid(
                entry["contacting_distribution"]
            )

        if "contact_gt" in entry:
            entry["contacting_gt"] = entry["contact_gt"]
        if "spatial_gt" in entry:
            entry["spatial_gt"] = entry["spatial_gt"]
        if "attention_gt" in entry:
            entry["attention_gt"] = entry["attention_gt"]
        if "contacting_distribution" in entry:
            entry["contact_distribution"] = entry["contacting_distribution"]
