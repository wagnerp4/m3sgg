...
def validate_config(
    config: Union[DictConfig, Dict[str, Any]], model_type: str
) -> List[str]:
    ...
    if "lr" in config:
        ...
        if not isinstance(lr, (int, float)) or lr <= 0 or lr > 1:
            errors.append(
                f"Invalid learning rate: {lr}. Must be a positive number <= 1"
            )
    ...
    if "nepoch" in config:
        ...
        if not isinstance(nepoch, (int, float)) or nepoch <= 0:
            errors.append(
                f"Invalid number of epochs: {nepoch}. Must be a positive number"
            )
