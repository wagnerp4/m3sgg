...
class DetectorConfig:
    ...
    resnet_backbone: str = (
        "resnet50"  # resnet18, resnet34, resnet50, resnet101, resnet152
    )
    ...
    def validate_config(self) -> bool:
        ...
        if self.detector_type not in valid_detector_types:
            raise ValueError(
                f"Invalid detector_type: {self.detector_type}. Must be one of {valid_detector_types}"
            )

        if (
            self.detector_confidence_threshold < 0
            or self.detector_confidence_threshold > 1
        ):
