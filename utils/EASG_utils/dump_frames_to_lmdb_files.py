...
class PyAVSCODDataset(Dataset):
    def __init__(
        ...
        for ann in annotations:
            ...
            for frameType in ["pre", "pnr", "post"]:
                last_frame = ann[f"{frameType}_frame"]["frame_number"]
                ...
                for chunk in frame_chunks:
                    ...
                    if len(chunk) <= max_chunk_size:
                        ...
                        for chunk in np.array_split(
                            chunk, np.ceil(len(chunk) / max_chunk_size)
                        ):
                            self.chunks.append((k, chunk))
                            total_frames += len(
                                chunk
        ...
        print(
            "Sampled {} chunks / {} frames in total".format(
                len(self.chunks), total_frames
            )
        )
    ...
    def __getitem__(self, index):
        ...
        for i in range(self.retry):
            ...
            vr = PyAVVideoReader(
                str(self.path_to_videos / (video_id + ".mp4")), height=args.frame_height
            )
...
if args.split > 0:
    ...
    video_uids = sorted(set([ann["video_uid"] for ann in annotations]))
    num_split = args.num_split
    size_split = len(video_uids) // num_split
    if args.split < num_split:
        video_uids = video_uids[
            size_split * (args.split - 1) : size_split * (args.split)
        ]
    elif args.split == num_split:
        video_uids = video_uids[size_split * (args.split - 1) :]

    annotations = [ann for ann in annotations if ann["video_uid"] in video_uids]
else:
...
dset = PyAVSCODDataset(
    annotations, args.path_to_videos, existing_keys=l.get_existing_keys()
)
dloader = DataLoader(
