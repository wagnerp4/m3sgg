...
parser.add_argument(
    "--fname_format", type=str, default="{video_uid:s}_{frame_number:07d}.jpg"
)
    ...
    for ann in tqdm(anns["clips"]):
        ...
        for frameType in ["pre", "pnr", "post"]:
            fname = args.fname_format.format(
                video_uid=video_id,
                frame_number=ann[f"{frameType}_frame"]["frame_number"],
            )
            ...
            for box, score, noun in zip(
                outputs.pred_boxes.tensor, outputs.scores, outputs.pred_classes
            ):
                box = box.tolist()
