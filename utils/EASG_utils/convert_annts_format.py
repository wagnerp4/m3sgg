...
def preprocess_from_list_to_dict(list_data):
    ...
    for data in list_data:
        ...
        for frameType in ["pre", "pnr", "post"]:
            dict_data[clip_uid][pnrFrameNumber][frameType] = process_bboxes(
                data, frameType
            )
...
if __name__ == "__main__":
    ...
    data_ego4d = read_ego4d_annts(file_annts_ego4d_train) + read_ego4d_annts(
        file_annts_ego4d_val
    )
    ...
    for annt_file in annt_files:
        ...
        assert (
            split == annts_ego4d["pre"]["split"]
            and split == annts_ego4d["post"]["split"]
        )
        if split == "train":
        ...
        if graph_uid not in new_annts:
            ...
            for frameType in ["pre", "pnr", "post"]:
                new_annts[graph_uid]["clip_frame_number"][frameType] = annts_ego4d[
                    frameType
                ]["clip_frame_number"]
        ...
        if "verb" in annts:
            ...
            newverb = (
                annts["newverb"]
                if "newverb" in annts and annts["newverb"] is not None
                else None
            )
            newnoun = (
                annts["newnoun"]
                if "newnoun" in annts and annts["newnoun"] is not None
                else None
            )
        ...
        if num_bbox:
            new_annts[graph_uid]["annotations"][aid].append(
                {"obj": dobj, "bbox": bbox_dict, "verb": verb, "rel": "dobj"}
            )
            ...
            if len(role_obj["frames"]):
                ...
                for frameType in ["pre", "pnr", "post"]:
                    ...
                    for indir_obj in [
                        ...
                        matched_noun = get_matched_noun(
                            indir_obj, annt_noun_bbox.keys()
                        )
                        if matched_noun:
                            for bbox in annt_noun_bbox[matched_noun]:
                                bbox_dict[frameType].append(bbox)
        ...
        for graph_uid in new_annts:
            for aid in new_annts[graph_uid]["annotations"]:
                for i in range(len(new_annts[graph_uid]["annotations"][aid])):
                    for frameType in new_annts[graph_uid]["annotations"][aid][i][
                        "bbox"
                    ]:
                        new_annts[graph_uid]["annotations"][aid][i]["bbox"][
                            frameType
                        ] = np.array(
                            new_annts[graph_uid]["annotations"][aid][i]["bbox"][
                                frameType
