...
parser.add_argument(
    "--fname_format", type=str, default="{clip_uid:s}_{clip_frame_number:06d}.jpg"
)
...
for ann in [train, val, test]:
    for x in ann["clips"]:
        ...
        for frameType in ["pre", "pnr", "post"]:
            fname = args.fname_format.format(
                clip_uid=clip_id,
                clip_frame_number=x[f"{frameType}_frame"]["clip_frame_number"],
            )
            names.append(fname)
...
print(
    f"Found {len(set(names))} frames to extract"
)  # 1363204 for all / 1361338 for all_vid
...
def process_video(cid, frames, path_to_videos, path_to_output, fname_format):
    ...
    for vf, fn in zip(video_frames, frames):
        imname = str(
            path_to_output / fname_format.format(clip_uid=cid, clip_frame_number=fn)
        )
        cv2.imwrite(imname, vf)
